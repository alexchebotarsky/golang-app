...
func (c *Client) FetchExampleData(ctx context.Context) (ExampleData, error) {
	...
	if err != nil {
		return ExampleData{}, fmt.Errorf("error creating new request: %v", err)
	}

	res, err := c.HTTPClient.Do(req)
	if err != nil {
		return ExampleData{}, fmt.Errorf("error doing http request: %v", err)
	}

	if res.StatusCode >= 400 {
		return ExampleData{}, fmt.Errorf("received non-successful status code: %s", res.Status)
	}

	var exampleData ExampleData
	err = json.NewDecoder(res.Body).Decode(&exampleData)
	if err != nil {
		return ExampleData{}, fmt.Errorf("error decoding example data: %v", err)
	}
